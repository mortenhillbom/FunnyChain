  ...
  class message {
            ...
            <div class='sli_media'><span class='sli_label'>Media</span>";
              ...
              if ($q->rowCount() > 0) {
                while ($row = $q->fetch(PDO::FETCH_OBJ)) {
                  ...
                  echo "<img src='{$this->DIR}/message/FunnyChain_{$row->message}' class='sli_media_img' data-description='{$sent}' title='By {$sent}' data-time='{$Time->timeAgo($row->time)}' data-imgby='{$universal->GETsDetails($row->mssg_by, "username")}'>";
                }
    ...
    public function grpConInfo($grp){
      ...
      echo
        ...
        if ($av == "") {
          ...
          echo "{$this->DIR}/grp_mssg_avatar/FunnyChain_{$av}";
        }
          ...
          <span class='sli_bold'>{$name}</span>
            ...
            <div class='sli_media'><span class='sli_label'>Media</span>";
              ...
              if ($q->rowCount() > 0) {
                while ($row = $q->fetch(PDO::FETCH_OBJ)) {
                  if ($row->mssg_by == $session) { $sent = "You"; } else { $sent = $universal->GETsDetails($row->mssg_by, "username"); }
                  echo "<img src='{$this->DIR}/message/FunnyChain_{$row->message}' class='sli_media_img' data-description='{$sent}' title='By {$sent}' data-time='{$Time->timeAgo($row->time)}' data-imgby='{$sent}'>";
                }
              } else if ($q->rowCount() == 0) {
                echo "<div class='home_last_mssg sli_last_mssg'><img src='{$this->DIR}/images/needs/tumblr_static_cbyn77qow2ogcgskwcko04c8w.png'></div>";
              }
              ...
              if ($type == "text" || $type == "image" || $type == "sticker") {
                ...
                echo"'>
                  ...
                  if ($type == "text") {
                    ...
                    echo "<img src='{$this->DIR}/message/FunnyChain_{$data}' class='m_m_img' data-imgby='{$m}' data-time='{$Time->timeAgo($time)}'>";
                  } else if ($type == "sticker") {
                    echo "<img src='{$this->DIR}/message/FunnyChain_{$data}' class='m_m_sticker'>";
                  }
                  echo "</div>
    ...
    public function sendMessageImage($file, $to, $con, $mssgOf){
      ...
      if (in_array($ext, $allowed)) {
        if ($error == 0) {
          $new_name = time().".".$ext;
          if (move_uploaded_file($tmp_name, "../../message/FunnyChain_".$new_name)) {
    ...
    public function sendMessageSticker($file, $touser, $con, $mssgOf){
      ...
      $to = "../../message/FunnyChain_".time().".".$ext;
      @copy($from, $to);
    ...
    public function deleteAllMssg($con, $by){
      ...
      if ($query->rowCount() > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          $mssg = $row->message;
          if ($type != "text") {
            if(file_exists("../../message/FunnyChain_{$mssg}")){
              unlink("../../message/FunnyChain_".$mssg);
            }
          }
        }
      }
    ...
    public function deleteConversation($con, $by){
      ...
      if ($by == "user") {
        ...
        $dir = "../../grp_mssg_avatar/FunnyChain_{$av}";
        if (file_exists($dir)) {
          unlink($dir);
        }
    ...
    public function deleteMessage($mssg, $con, $type, $by){
      ...
      if ($type != "text") {
        ...
        if ($q->rowCount() > 0) {
          $row = $q->fetch(PDO::FETCH_OBJ);
          $data = $row->message;
          if (file_exists("../../message/FunnyChain_{$data}")) {
            unlink("../../message/FunnyChain_".$data);
          }
        }
      }
      if ($by == "user") {
    ...
    public function addGroup($cname, $members, $file){
      ...
      if (isset($file)) {
        ...
        if (in_array($ext, $allowed)) {
          $new_name = time().".".$ext;
          move_uploaded_file($tmp_name, "../../grp_mssg_avatar/FunnyChain_".$new_name);
        }
      }
      ...
      if ($query->rowCount() != 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          if ($av == "") {
            ...
            echo "<img src='{$this->DIR}/grp_mssg_avatar/FunnyChain_{$av}'>";
          }
            echo "<div class='m_sr_ontent'>
    ...
    public function grpConAvatar($av){
      if ($av == "") {
        ...
        return "{$this->DIR}/grp_mssg_avatar/FunnyChain_".$av;
      }
    }
    ...
    public function changeGrpConAvatar($file, $grp){
      ...
      if ($query->rowCount() > 0) {
        ...
        if ($av != "") {
          unlink("../../grp_mssg_avatar/FunnyChain_$av");
        }

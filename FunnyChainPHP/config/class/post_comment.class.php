  ...
  class postComment{
    ...
    public function imageComment($file, $post){
      ...
      if($settings->AmIBlocked($to) == false){
        if (in_array($ext, $allowed)) {
          if ($error == 0) {
            $new_name = time().".".$ext;
            if (move_uploaded_file($tmp_name, "../../comments/FunnyChain_".$new_name)) {
      ...
      if ($count > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          if ($type == "text") {
            ...
            $return = "<img src='{$this->DIR}/comments/FunnyChain_{$data}' class='comments_img' data-imgby='{$z}' data-time='{$Time->timeAgo($time)}'>";

          } else if ($type == "sticker") {
            $return = "<img src='{$this->DIR}/comments/FunnyChain_{$data}' class='comment_sticker'>";
          }
    ...
    public function commSticker($sticker, $post){
      ...
      if($settings->AmIBlocked($to) == false){
        ...
        $to = "../../comments/FunnyChain_".time().".".$ext;
